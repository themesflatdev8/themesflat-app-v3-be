...
class TAuthService extends AbstractAuthService
{
    ...
    public  function verifyRequest($data, $force = false)
    {
        ...
        $secret = config('fa_common.shopify_api_secret');
        ...
        SystemCache::addItemToHash(config('fa_setting.cache.list_store_key'), $store->store_id, $store->shopify_domain);
        SystemCache::addItemToHash(config('fa_setting.cache.store_detail_key'), $store->shopify_domain, $store->toArray());
        $data = $store->toArray();
    ...
    public function verifyCallback($data)
    {
        ...
        info("Tokennnnnnnnnnnn: {$token}");
        $shop = StoreModel::where('shopify_domain', $data['shop'])->orWhere('domain', $data['shop'])->first();
        ...
        if ($userType != 'active') {
            ...
            if ($shop->app_version != config('fa_common.app_version')) {
                event(new UpdateVersionAppEvent([
                    'shop' => $shop,
                    'token' => $token
                ]));
